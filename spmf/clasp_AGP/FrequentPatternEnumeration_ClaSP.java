...
import spmf.clasp_AGP.dataStructures.Item;
import spmf.clasp_AGP.dataStructures.abstracciones.ItemAbstractionPair;
...
import spmf.clasp_AGP.dataStructures.creators.ItemAbstractionPairCreator;
import spmf.clasp_AGP.dataStructures.patterns.Pattern;
import spmf.clasp_AGP.idlists.IDList;
import spmf.clasp_AGP.savers.Saver;
import spmf.clasp_AGP.tries.Trie;
import spmf.clasp_AGP.tries.TrieNode;
...
public class FrequentPatternEnumeration_ClaSP {
    ...
    private void exploreChildren(Pattern pattern, TrieNode currentNode, List<TrieNode> sequenceExtensions,
        ...
        if (!isAvoidable) {
            ...
            loops: for (TrieNode node : sequenceExtensions) {
        ...
        /*
         * We clear the set of the new pattern and the nodes associated with them since
         * discovered since we need to store those that we will find making i-extensions
         */
        newPatterns.clear();
         ...
         */
        loopi: for (int k = beginning; k < itemsetsExtensions.size(); k++) {
        ...
        for (int i = 0; i < itemsetExtensionSize; i++) {
            ...
            exploreChildren(newPattern, nodeToExtend, new_sequenceExtension, new_itemsetExtension, i + 1, coocMapAfter,
                    coocMapEquals, last);
