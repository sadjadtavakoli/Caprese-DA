...
import spmf.clasp_AGP.savers.SaverIntoFile;
import spmf.clasp_AGP.savers.SaverIntoMemory;
import spmf.clasp_AGP.tries.Trie;
import spmf.tools.MemoryLogger;
        ...
        for (Sequence seq : database.getSequences()) {
            ...
            loopseq: for (int i = 0; i < seq.getItemsets().size(); i++) {
                ...
                for (int j = 0; j < itemsetA.size(); j++) {
                    ...
                    /**
                     * 
                     * @TODO @sadjad here is where we should apply our changes one of these change can be
                     *         just considering items before the last appearance of our change-set
                     *         items. So the first iteration is not to the sequence length, it's
                     *         just to the last apearance position
                     */
